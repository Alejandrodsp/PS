            ...
            for (File bin : bins){
                ...
                if (controle > 1){
                    while (reader.hasNext()){
                        ...
                        if (line.length() <= 16){ //INTRUÇÕES DE 2 BYTES
                            size_incremento += line.length() / 8;
                            escrever.write(line);
                            escrever.newLine();
                        } else {
                            if (line.charAt(6) == '0' && line.charAt(7) == '1'){ //INSTRUÇÕES IMEDIATAS
                                size_incremento += line.length() / 8;
                                escrever.write(line);
                                escrever.newLine();
                            } else if (line.startsWith("W")){ //UMA WORD
                                size_incremento += line.replace("W", "").length() / 8;
                                escrever.write(line);
                                escrever.newLine();
                            } else { // CASO CONTRARIO PRECISA INCREMENTAR
